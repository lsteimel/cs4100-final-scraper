...
import { TwitterGuestAccount } from './auth/guest.ts';
import { 
    TwitterConstructor, 
...
export class Twitter {
    ...
    private credentials: TwitterGuestAccountCredentials[] = [];
    ...
    public async createCredentials(amount: number, path?: string): Promise<void> {
        for (let i = 0; i < amount; i++) {
            console.log(`Creating guest credentials ${i + 1}/${amount}`);
            ...
            try {
                ...
                console.error(`Failed to create guest credentials ${i + 1}/${amount}`, e);
            }
        }
    ...
    public async getThread(id: string): Promise<GetThreadResponse> {
